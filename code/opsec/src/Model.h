 ...
 * must implement.  Correlation functions are abstracted by the XiFunc struct,
 * for which each model should provide its own XiFuncImpl implementation (see
 * RealModel.cpp for an example).  Note that if you add your own model
 * subclass, you should update the InitializeModel() method in Model.cpp! */

struct XiFuncImpl {
    /* Subclasses must implement the following method. */
    virtual double xi(double r, double a, double b) const = 0;
...
public:
    ...
    double operator()(double r, double a = 0, double b = 0) const;
 ...
 * function $\xi(\vec{r})$ depends on a set of parameters $p_n$.  A concrete
 * implementation of the abstract class Model must define the following
 * methods:
 *  int NumParams();
...
public:
    ...
    virtual XiFunc GetXi() = 0;                 // get the model 2-point function \xi(r,a,b)
    virtual XiFunc GetXiDeriv(int n) = 0;       // get the partial derivative d\xi(r,a,b)/dp_n
};
