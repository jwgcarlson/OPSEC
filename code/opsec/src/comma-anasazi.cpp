...
//    }
    ...
    /* Determine the local problem size for each process */
    ...
    int astart = locdisps[me];

    /* Read mode file header */
    FILE* fmodes = NULL;
    std::vector<real> v;        // storage for full mode on root process
    if(me == 0) {
        int Nmodes_check, Ncells_check;
        ...
        if(fmodes == NULL)
            opsec_abort(1);
        if(Nmodes != Nmodes_check || Ncells != Ncells_check) {
            fprintf(stderr, "comma: inconsistent modes file (%d,%d;%d,%d)\n", Nmodes, Nmodes_check, Ncells, Ncells_check);
            opsec_abort(1);
        }
        v.resize(Ncells);
    }

    /* Multi-vector for storing mode coefficients matrix B */
    MyMultiVec<real> B(Ncells, Nmodes);

    /* Read in modes, one full mode at a time, distributing the appropriate
     * elements to each process. */
    for(int i = 0; i < Nmodes; i++) {
        if(me == 0) {
            size_t nread = fread(&v[0], sizeof(real), Ncells, fmodes);
            if(nread != Ncells) {
                fprintf(stderr, "comma: mode file truncated (%d,%d,%zd)\n", i, Ncells, nread);
                opsec_abort(1);
            }
        }
#ifdef OPSEC_USE_MPI
        MPI_Scatterv(&v[0], &locsizes[0], &locdisps[0], REAL_MPI_TYPE, B.vector(i), REAL_MPI_TYPE, 0, MPI_COMM_WORLD);
#else
        memcpy(B.vector(i), &v[0], Ncells*sizeof(real));
#endif
    }

    /* Multi-vector for storing the product matrix C = S . B */
    MyMultiVec<real> C(Ncells, Nmodes);

    /* Multi-vector for storing blocks of the signal matrix, Sblock */
    MyMultiVec<real>* Sblock_storage = new MyMultiVec<real>(nloc, Nmodes);

    /* Matrix for storing blocks of the product matrix Cblock = Sblock^T . B */
    Teuchos::SerialDenseMatrix<int,real> Cblock(nloc, Nmodes);

    /* Final signal matrix */
    Teuchos::SerialDenseMatrix<int,real> Sp(Nmodes, Nmodes);

    for(int m = 0; m < Nparams; m++) {

        int ncomputed = 0;      // number of rows of signal matrix that have been computed
        while(ncomputed < Ncells) {
            /* Size of block to compute, taking into account the possibility
             * that the starting blockwidth is too large */
            int nblock = (ncomputed + blockwidth > Ncells) ? Ncells - ncomputed : blockwidth;

            /* Clone a view into the storage multi-vector */
            MyMultiVec<real>* Sblock = Sblock_storage->CloneViewNonConst(irange(nblock));

            /* Compute the signal matrix elements */
            for(int aloc = 0; aloc < nloc; aloc++) {
                int a = aloc + astart;
                for(int bblock = 0; bblock < nblock; bblock++) {
                    int b = bblock + ncomputed;

                    /* Element (aloc,bblock) of the local block represents
                     * element (a,b) of the global signal matrix */
                    Sblock->local(aloc,bblock)
                        = ComputeSignalMatrixElement(a, b, xip[m], cells, coordsys);
                }
            }

            /* Multiply Sblock^T . B */
            B.MvTransMVReduce(1, *Sblock, Cblock, p);

            /* Fill in elements of C */
            for(int aloc = 0; aloc < nloc; aloc++)
                for(int bblock = 0; bblock < nblock; bblock++)
                    C.local(aloc, bblock) = Cblock(aloc, bblock);

            delete Sblock;
            ncomputed += nblock;
        }

        /* Get final signal matrix in mode basis, S' = B^T . S . B */
    }

    /* Free unused memory */
    delete Sblock_storage;
    Cblock.reshape(1,1);
...
#ifdef OPSEC_USE_MPI
            MPI_Bcast(mode, Ncells, REAL_MPI_TYPE, 0, MPI_COMM_WORLD);
#endif
...
#ifdef OPSEC_USE_MPI
        if(me == 0)
            MPI_Reduce(MPI_IN_PLACE, Sp, Nmodes*Nmodes, REAL_MPI_TYPE, MPI_SUM, 0, MPI_COMM_WORLD);
        else
            MPI_Reduce(Sp, NULL, Nmodes*Nmodes, REAL_MPI_TYPE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
