...
struct ConstantSplineImpl : public SplineImpl {
    double value;

    ConstantSplineImpl(double value_, double xmin_, double xmax_) {
        value = value_;
        xmin = xmin_;
        xmax = xmax_;
    }
...
Spline ConstantSpline(double value, double xmin, double xmax) {
    return Spline(new ConstantSplineImpl(value, xmin, xmax));
}
...
/* FIXME:
 * - have a default implementation that always returns 0, so that impl is never NULL
 * - avoid problems with self-assignment in operator=() */
