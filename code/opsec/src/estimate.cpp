...
/* Read signal-to-noise eigenvalues from file. */
real* ReadEvals(const char* evalfile, int* Nmodes_ = NULL) {
    ...
    if(err || evals == NULL) {
        fprintf(stderr, "ReadEvals: error reading from '%s'\n", evalfile);
    }
    else {
...
/* Note: This routine leaks memory if unsuccessful. */
real** ReadDerivatives(const char* covfile, bool multifile, int* Nparams_ = NULL, int* Nmodes_ = NULL) {
    int Nparams = 0;
    int Nmodes = 0;
    real** Cp = NULL;
    FILE* fcov = NULL;
...
int main(int argc, char* argv[]) {
    ...
    if(!cfg_has_keys(cfg, "estfile,evalfile,covfile,pixfile", ",")) {
    ...
    /* Read signal-to-noise eigenvalues $\lambda_i$ */
    int Nmodes;
    real* c = ReadEvals(evalfile, &Nmodes);
    if(c == NULL) {
        fprintf(stderr, "estimate: error reading signal-to-noise eigenvalues\n");
        opsec_exit(1);
    }
