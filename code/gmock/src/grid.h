...
struct Grid {
    ...
    fftw_real& r(int ixloc, int iy, int iz) {
        return data[(ixloc*ny + iy)*(2*(nz/2+1)) + iz];
    }

    fftw_complex& c(int ixloc, int iy, int iz) {
        return ((fftw_complex*)data)[(ixloc*ny + iy)*(nz/2+1) + iz];
    }

    fftw_real& r_t(int ix, int iyloc, int iz) {
        return data[(iyloc*nx + ix)*(2*(nz/2+1)) + iz];
    }

    fftw_complex& c_t(int ix, int iyloc, int iz) {
        return ((fftw_complex*)data)[(iyloc*nx + ix)*(nz/2+1) + iz];
    }

    /* Shorthand for most common case */
    fftw_real& operator()(int ixloc, int iy, int iz) {
